...
window.onload = () => {
    const stored = localStorage.getItem('driveSewaCustomer') || localStorage.getItem('driveSewaUser');
    if(!stored) {
        window.location.href = '../../public/login.html';
        return;
    }
    customerData = JSON.parse(stored);
    if(!customerData.role || customerData.role.toLowerCase() !== 'customer') {
        logout();
        return;
    }
...
async function loadPartRequests() {
    try {
        const res = await fetch(`${apiBaseUrl}/partrequests/customer/${customerId}`);
        ...
        container.innerHTML = `
            <table>
                <thead>
                    <tr><th>Part Name</th><th>Vehicle Details</th><th>Qty</th><th>Description</th><th>Requested At</th><th>Status</th></tr>
                </thead>
                <tbody>
                    ${requests.map(r => `
                        <tr>
...
document.getElementById('partRequestForm').addEventListener('submit', async (e) => {
    ...
    const payload = {
        ...
        quantityNeeded: parseInt(document.getElementById('quantityNeeded').value, 10),
